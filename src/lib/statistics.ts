...
export function calculateFrequencies(draws: DrawData[]): NumberFrequency[] {
  const frequencies: Map<number, { count: number; lastDrawn: number }> =
    new Map();
  ...
  const latestDraw =
    draws.length > 0 ? Math.max(...draws.map((d) => d.numero)) : 0;

  // Convert to array with percentages
  return Array.from(frequencies.entries()).map(
...
export function getHotNumbers(
  ...
  return [...frequencies].sort((a, b) => b.count - a.count).slice(0, n);
}
...
export function getColdNumbers(
  ...
  return [...frequencies].sort((a, b) => a.count - b.count).slice(0, n);
}
...
export function getOverdueNumbers(
  ...
  return [...frequencies].sort((a, b) => b.gap - a.gap).slice(0, n);
}

/**
 * Calculate average gap between appearances for each number
 */
export function calculateAverageGaps(
  draws: DrawData[]
