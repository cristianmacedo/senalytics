...
export function History() {
  ...
  const { data: drawDetails, isLoading: isLoadingDetails } = useDraw(selectedDraw ?? 0);
    ...
    <div className="space-y-8">
      ...
      >
        ...
        <div className="space-y-4">
          {paginatedDraws.map((draw) => (
            <Card 
              ...
              onClick={() => setSelectedDraw(selectedDraw === draw.numero ? null : draw.numero)}
            >
              <div className="flex flex-col md:flex-row md:items-center justify-between gap-4">
                <div className="flex items-center gap-4">
                  <div className="text-center">
                    <p className="text-xs text-slate-500 uppercase">Concurso</p>
                    <p className="text-2xl font-bold text-caixa-blue">{draw.numero}</p>
                  </div>
                  <div className="h-12 w-px bg-slate-200" />
                  <div>
                    <p className="text-sm text-slate-500">{formatDateBR(draw.data)}</p>
                    <div className="flex items-center gap-2 mt-1">
                      ...
                      {draw.ganhadores[0] > 0 && (
                        <span className="text-xs bg-mega-green/10 text-mega-green px-2 py-0.5 rounded-full font-medium">
                          {draw.ganhadores[0]} ganhador{draw.ganhadores[0] > 1 ? 'es' : ''}
                        </span>
                      )}
                    </div>
                  </div>
                ...
                <div className="text-right">
                  ...
                  <p className="font-bold text-mega-green">{formatBRL(draw.premioSena)}</p>
                </div>
              </div>
                ...
                <div className="mt-4 pt-4 border-t border-slate-100">
                  {isLoadingDetails ? (
                    ...
                    <div className="grid grid-cols-1 md:grid-cols-3 gap-4">
                      <div>
                        <p className="text-xs text-slate-500 uppercase mb-2">Premiação</p>
                        <div className="space-y-1">
                          {drawDetails.listaRateioPremio.map((premio) => (
                            <div key={premio.faixa} className="flex justify-between text-sm">
                              <span className="text-slate-600">{premio.descricaoFaixa}</span>
                      ...
                      <div>
                        <p className="text-xs text-slate-500 uppercase mb-2">Local do Sorteio</p>
                        <p className="text-sm text-slate-900">{drawDetails.localSorteio}</p>
                        <p className="text-sm text-slate-600">{drawDetails.nomeMunicipioUFSorteio}</p>
                      </div>
                      
                      <div>
                        <p className="text-xs text-slate-500 uppercase mb-2">Arrecadação</p>
        ...
        <p className="text-center text-sm text-slate-500 mt-4">
          Mostrando {page * ITEMS_PER_PAGE + 1} - {Math.min((page + 1) * ITEMS_PER_PAGE, filteredDraws.length)} de {filteredDraws.length.toLocaleString('pt-BR')} resultados
        </p>
      </Section>
    </div>
  );
