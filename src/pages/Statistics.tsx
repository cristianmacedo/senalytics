...
export function Statistics() {
  ...
  return (
    <div className="space-y-8">
      ...
      {/* Frequency Chart */}
      <Section title="Frequência de Números" subtitle="Quantas vezes cada número foi sorteado">
        <Card>
          <ResponsiveContainer width="100%" height={300}>
            <BarChart data={stats.frequencyChartData}>
              ...
              <Tooltip 
                ...
                formatter={(value: number) => [`${value} vezes`, 'Sorteado']}
              ...
              <Bar dataKey="count" radius={[4, 4, 0, 0]}>
                {stats.frequencyChartData.map((entry, index) => (
                  <Cell
                    key={`cell-${index}`}
                    fill={entry.isHot ? '#209869' : entry.isCold ? '#ef4444' : '#94a3b8'}
                  />
                ))}
              </Bar>
            </BarChart>
            ...
            <div className="space-y-3">
              {stats.hotNumbers.map((num) => (
                <div key={num.number} className="flex items-center justify-between">
                  <LotteryBall number={num.number} size="sm" variant="default" />
                  <div className="flex-1 mx-4">
                    <div className="h-2 bg-slate-100 rounded-full overflow-hidden">
                      <div 
                        className="h-full bg-mega-green rounded-full transition-all"
                        style={{ width: `${(num.count / stats.hotNumbers[0].count) * 100}%` }}
                      />
                    </div>
                  </div>
          ...
          <Card>
            <div className="space-y-3">
              {stats.overdueNumbers.map((num) => (
                <div key={num.number} className="flex items-center justify-between">
                  <LotteryBall number={num.number} size="sm" variant="highlight" />
                  <div className="flex-1 mx-4">
                    <div className="h-2 bg-slate-100 rounded-full overflow-hidden">
                      <div 
                        className="h-full bg-amber-400 rounded-full transition-all"
                        style={{ width: `${(num.gap / stats.overdueNumbers[0].gap) * 100}%` }}
                      />
                    </div>
                  </div>
      ...
      <Section title="Padrões" subtitle="Distribuição de características">
        <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
          ...
          {/* Consecutive */}
          <Card>
            <h3 className="font-bold text-slate-900 mb-4">Números Consecutivos</h3>
            <div className="space-y-4">
              ...
              <div className="h-4 bg-slate-100 rounded-full overflow-hidden flex">
                ...
                <div 
                  ...
                  style={{ width: `${100 - stats.consecutive.percentageWith}%` }}
                />
              </div>
              <p className="text-xs text-slate-500">
                {stats.consecutive.withConsecutive.toLocaleString('pt-BR')} concursos tinham ao menos dois números consecutivos
              </p>
            </div>
          </Card>
        </div>
            ...
            <table className="w-full">
              <thead>
                <tr className="border-b border-slate-200">
                  <th className="text-left py-3 px-4 font-semibold text-slate-900">Acertos</th>
                  <th className="text-right py-3 px-4 font-semibold text-slate-900">Probabilidade</th>
                  <th className="text-right py-3 px-4 font-semibold text-slate-900">1 em</th>
                </tr>
              </thead>
              <tbody>
                {[6, 5, 4, 3, 2, 1, 0].map((hits) => {
                  ...
                  return (
                    <tr key={hits} className="border-b border-slate-100 last:border-0">
                      <td className="py-3 px-4">
                        <span className={`font-medium ${hits >= 4 ? 'text-mega-green' : 'text-slate-600'}`}>
                          {hits} números
                        </span>
                        {hits === 6 && <span className="ml-2 text-xs text-amber-500 font-bold">SENA</span>}
                        {hits === 5 && <span className="ml-2 text-xs text-slate-400 font-bold">QUINA</span>}
                        {hits === 4 && <span className="ml-2 text-xs text-slate-400 font-bold">QUADRA</span>}
                      </td>
                      <td className="py-3 px-4 text-right font-mono text-sm">
                        {(prob * 100).toFixed(6)}%
                      </td>
                      <td className="py-3 px-4 text-right font-medium">
                        {odds === Infinity ? '—' : Math.round(odds).toLocaleString('pt-BR')}
                      </td>
                    </tr>
                  );
                })}
