...
export function Simulator() {
  ...
  const handleSimulate = () => {
    ...
    const result = simulateAgainstHistory(selectedNumbers, historicalData.draws);
        ...
        <Card>
          <div className="space-y-6">
            ...
            <div className="flex flex-wrap gap-3 justify-center">
              ...
              <Button onClick={handleClear} variant="ghost" disabled={selectedNumbers.length === 0}>
                Limpar
              </Button>
              <Button 
      ...
      {simulationResult && (
        ...
        >
          ...
          <div className="grid grid-cols-2 md:grid-cols-4 gap-4 mb-6">
            <Card className={simulationResult.results.matches6 > 0 ? 'bg-gradient-to-br from-amber-400 to-amber-500 text-white' : ''}>
              <div className="text-center">
                <p className="text-sm opacity-80">Sena (6)</p>
                <p className="text-3xl font-bold">{simulationResult.results.matches6}</p>
              </div>
            </Card>
            <Card className={simulationResult.results.matches5 > 0 ? 'bg-gradient-to-br from-mega-green to-mega-green-dark text-white' : ''}>
              <div className="text-center">
                <p className="text-sm opacity-80">Quina (5)</p>
                <p className="text-3xl font-bold">{simulationResult.results.matches5}</p>
              </div>
            </Card>
            <Card className={simulationResult.results.matches4 > 0 ? 'bg-gradient-to-br from-caixa-blue to-caixa-blue-dark text-white' : ''}>
              <div className="text-center">
                <p className="text-sm opacity-80">Quadra (4)</p>
                <p className="text-3xl font-bold">{simulationResult.results.matches4}</p>
              </div>
            </Card>
            <Card>
              <div className="text-center">
                ...
                <p className="text-3xl font-bold text-slate-400">
                  {simulationResult.results.matches3 + simulationResult.results.matches2 + simulationResult.results.matches1 + simulationResult.results.matches0}
                </p>
              </div>
            </Card>
          </div>
          ...
          {/* Distribution */}
          <Card className="mb-6">
            <h3 className="font-bold text-slate-900 mb-4">Distribuição de Acertos</h3>
            <div className="space-y-2">
              {[
                { label: '6 acertos', value: simulationResult.results.matches6, color: 'bg-amber-400' },
                { label: '5 acertos', value: simulationResult.results.matches5, color: 'bg-mega-green' },
                { label: '4 acertos', value: simulationResult.results.matches4, color: 'bg-caixa-blue' },
                { label: '3 acertos', value: simulationResult.results.matches3, color: 'bg-slate-400' },
                { label: '2 acertos', value: simulationResult.results.matches2, color: 'bg-slate-300' },
                { label: '1 acerto', value: simulationResult.results.matches1, color: 'bg-slate-200' },
                { label: '0 acertos', value: simulationResult.results.matches0, color: 'bg-slate-100' },
                ...
                <div key={label} className="flex items-center gap-3">
                  ...
                  <div className="flex-1 h-6 bg-slate-100 rounded-full overflow-hidden">
                    <div 
                      ...
                      style={{ width: `${(value / simulationResult.totalDraws) * 100}%` }}
                    />
                  </div>
                  ...
                  >
                    ...
                    <div className="text-right">
                      ...
                      {(() => {
                        ...
                        const prize = draw.premios?.[prizeIndex] 
                          ?? (matches === 6 ? (draw as unknown as { premioSena?: number }).premioSena : 0) 
                          ?? 0;
                        if (prize > 0) {
                          const colorClass = matches === 6 ? 'text-amber-600' : matches === 5 ? 'text-mega-green' : 'text-caixa-blue';
                          return (
